...
module "app_sg_80_http" {
  source = "terraform-aws-modules/security-group/aws//modules/http-80"

  name        = "${var.project_name}-app-sg-80-http"
  description = "Security group for web-server with HTTP ports open with aws public subnet"
  vpc_id      = aws_vpc.main.id

  ingress_cidr_blocks = [for s in aws_subnet.public : s.cidr_block]
  ingress_rules       = ["http-80-tcp"]
}
